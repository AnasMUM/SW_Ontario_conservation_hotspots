...
all<-read.csv("all_var_180621.csv")
all=na.omit(all)
sp<-all[,15:21]
human<-all[,22:39]
area=all$Area_KM2.x
...
proactive.index.1<-(stress.inverse*2)+(res.fish.SAR.ratio+res.Mussel.SAR.ratio)/2

proactive.index.1

proactive.index.2<-(stress.inverse*2)+(res.fish.rich+res.mussel.rich)/2
...
#Representative
representative.index<-(res.fish.rich+res.mussel.rich)/2+(res.fish.SAR.ratio+res.Mussel.SAR.ratio)/2
...
SWON<-st_read("SW_ON_Hybas_07_sel.shp")

glakes<-st_read("great_lakes_sel_reprojected.shp")

surround<-st_read("SW_ON_Hybas_07_with_surround_reprojected_120721.shp")

CN<-st_read("CN_boundary_2.shp")

border<-st_read("Hybas7_study_wshds.shp")

CN_Pr<-st_read("CN_provinces_reprojected.shp")
...
tm.proactive.1<-tm_shape(surround_lmb,  bbox=sel)+
  ...
  tm_layout(legend.bg.color = "white", legend.bg.alpha = 1, legend.position=c("right" , "bottom"), legend.text.size=0.8, 
            legend.title.size = 1,legend.title.fontface ="bold", panel.show = T, panel.labels = "Proactive Template 1", 
...
tm.proactive.2<-tm_shape(surround_lmb,  bbox=sel)+
  ...
  #tm_borders(lwd = 0.65)+
  tm_layout(legend.bg.color = "white", legend.bg.alpha = 1, legend.position=c("right" , "bottom"), legend.text.size=0.8, 
            legend.title.size = 1,legend.title.fontface ="bold", panel.show = T, panel.labels = "Proactive Template 2", 
...
tm.proactive.3<-tm_shape(surround_lmb,  bbox=sel)+
  ...
  tm_layout(legend.bg.color = "white", legend.bg.alpha = 1, legend.position=c("right" , "bottom"), legend.text.size=0.8, legend.width=0.4,legend.height = 0.3,
            legend.title.size = 1,legend.title.fontface ="bold", panel.show = T, panel.labels = "Proactive Template 3", 
            panel.label.fontface="bold", panel.label.size = 1.2,  between.margin = 30,  inner.margins=0.00,outer.margins = c(0.02, -0.1, 0.02, 0))+
  #tm_graticules(labels.show=c(T, T), ticks = FALSE, lines=F, labels.size = 0.6)+
  tm_shape(glakes_lmb)+
  tm_polygons(lwd = 0.8, 
